...
class PlayerClient(BaseClient):
    ...
    async def get_legend_player(self, player: coc.Player):
        ...
        if data is None:
            await self.bot.player_stats.update_one({"tag": player.tag}, {"$set": {"paused": False, "name" : player.name, "tag" : player.tag, "league": player.league.name}}, upsert=True)
    ...
    async def get_clan_legend_players(self, clan: coc.Clan):
        ...
        for tag, member in tag_map.items():
            ...
            if data is None:
                await self.bot.player_stats.update_one({"tag": tag}, {"$set": {"paused": False, "name": member.name, "tag": member.tag, "league": member.league.name}}, upsert=True)
