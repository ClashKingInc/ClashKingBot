...
class OpenTicket():
    ...
    async def rename_ticket(self):
        ...
        types = {"{ticket_count}": self.number, "{user}": user.name if user is not None else "", "{account_name}": coc_account.name if coc_account is not None else "",
                 "{account_th}": coc_account.town_hall if coc_account is not None else "",
                 "{ticket_status}": self.status, "{emoji_status}": status_emoji[self.status]}
...
class BaseTicket():
    ...
    async def send_log(self, log_type: LOG_TYPE, user: disnake.User, ticket: OpenTicket = None, ticket_channel: disnake.TextChannel = None):
        ...
        if log_type == LOG_TYPE.BUTTON_CLICK:
            embed = disnake.Embed(description=f"**Button Clicked**\n"
            ...
            embed = disnake.Embed(description=f"**Status Changed | {ticket.status.capitalize()}**\n"
                                              f" - By: {user.mention}\n"
            ...
            if ticket.thread is not None:
                ...
                if thread_channel is not None:
                    ...
                    link = await upload_html_to_cdn(bytes_=io.BytesIO(transcript.encode()), id=f"transcript-{thread_channel.id}")
            ...
            link = await upload_html_to_cdn(bytes_=io.BytesIO(transcript.encode()), id=f"transcript-{ticket_channel.id}")
            link = f"https://api.clashking.xyz/renderhtml?url={link}"
...
class TicketPanel(BaseTicket):
    def __init__(self, bot: CustomClient, panel_settings):
        ...
        self.buttons: List[Ticket_Buttons] = [Ticket_Buttons(button_data=button_data | {"count" : count}, bot=bot, panel_settings=panel_settings)
    ...
    async def edit_button(self, button: str, new_text: str, new_color: str, new_emoji: str = None):
        ...
        new_button = disnake.ui.Button(label=new_text, emoji=new_emoji, style=text_style_conversion[new_color], custom_id=button.custom_id)
        await self.bot.tickets.update_one({"$and": [{"server_id": self.panel_server.id}, {"name": self.panel_name}]}, {"$unset": {f"components.{button.count_spot}": 1}})
        await self.bot.tickets.update_one({"$and": [{"server_id": self.panel_server.id}, {"name": self.panel_name}]}, {"$pull": {f"components": None}})
        await self.bot.tickets.update_one({"$and": [{"server_id": self.panel_server.id}, {"name": self.panel_name}]}, {"$push": {"components": new_button.to_component_dict()}})
...
class Ticket_Buttons(BaseTicket):
    def __init__(self, bot: CustomClient, panel_settings, button_data):
        ...
        self.message: Embed = Embed.from_dict(data=self.settings.get("message")) if self.settings.get("message") is not None \
            else Embed(description="This ticket will be handled shortly!\nPlease be patient.", color=disnake.Color.green())
    ...
    def button(self):
        ...
        return Button(label=self.label, emoji=emoji, style=style[self.style], custom_id=self.custom_id)
