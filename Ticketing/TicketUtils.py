...
async def open_ticket(bot: CustomClient, ticket_panel: TicketPanel, button: Ticket_Buttons, ctx: disnake.MessageInteraction, coc_account: MyCustomPlayer = None):
    ...
    channel_name = await naming_convention_convertor(bot=bot, user=ctx.user, naming_convention=button.naming_convention, coc_account=coc_account, guild=ctx.guild)
    ...
    if button.private_thread:
        thread = await channel.create_thread(name=f"Private | {channel_name.replace('-', ' ')}", type=disnake.ChannelType.private_thread)
...
async def naming_convention_convertor(bot: CustomClient, guild: disnake.Guild, user: disnake.User, naming_convention: str, number: int = None, status: str = "open", coc_account: MyCustomPlayer = None):
    ...
    types = {"{ticket_count}": number, "{user}": user.name, "{account_name}": coc_account.name if coc_account is not None else "",
...
async def message_convertor(bot: CustomClient, ctx: disnake.MessageInteraction, ticket: OpenTicket, message: str, custom_field: dict = None):
    status_emoji = {"open": "✅", "sleep": "🌙", "closed": "❌"}
