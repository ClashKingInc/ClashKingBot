    ...
    if print:
        buttons.append(
            disnake.ui.Button(
                ...
                emoji='🖨️',
                style=disnake.ButtonStyle.grey,
                custom_id=f'{button_id}:page=PRINT',
            )
        )
...
def clan_component(bot: CustomClient, all_clans: List[coc.Clan], clan_page: int = 0, max_choose=None):
    ...
    if clan_page >= 1:
        clan_options.append(disnake.SelectOption(label='< Previous 25 Clans', value=f'clanpage_{clan_page - 1}'))
    ...
    if len(clans) == length and (len(all_clans) > (length * clan_page) + length):
        clan_options.append(disnake.SelectOption(label='Next 25 Clans >', value=f'clanpage_{clan_page + 1}'))

    clan_select = disnake.ui.Select(
        options=clan_options,
        placeholder='Select Clan(s)',  # the placeholder text to show when no options have been chosen
