...
async def war_hitrate(
    ...
    for tag, data in hit_rate_dict.items():
        ...
        holder_list.append(
            holder(
                ...
                tag=tag,
                townhall=tag_to_th_map.get(tag),
    ...
    embed.set_author(
        name=f'{(clan or server).name} Top {limit} Hitrate',
        icon_url=get_guild_icon(server) if clan is None else clan.badge.url,
    )
    if num_wars is None and num_days is None:
        season = season or bot.gen_season_date()
...
async def war_hitcount(
    ...
    holder = namedtuple(
        'holder',
        ['name', 'tag', 'townhall', 'hitrate_0', 'hitrate_1', 'hitrate_2', 'hitrate_3'],
    )
    for tag, data in hit_rate_dict.items():
        total_attacks = data.get('total', 0)
        if total_attacks == 0:
