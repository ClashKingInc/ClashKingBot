...
async def main_war_page(bot: CustomClient, war: coc.ClanWar, war_league=None, is_previous=False):
	...
	embed.add_field(name="**War State**", value=state_text, inline=False)
...
async def create_components(bot: CustomClient, plans, war: coc.ClanWar):
	...
	player_select = disnake.ui.Select(
	    ...
	    placeholder="Set Plan for Players",  # the placeholder text to show when no options have been chosen
	...
	if player_options_two:
		player_select_two = disnake.ui.Select(
		    ...
		    placeholder="Set Plan for Players #2",  # the placeholder text to show when no options have been chosen
		    min_values=1,  # the minimum number of options a user must select
...
async def round_components(league_wars: List[coc.ClanWar]):
	options = [disnake.SelectOption(label="Overview", value="cwlround_overview")]
