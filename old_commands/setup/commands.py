...
from discord import convert, options
...
from utility.emojis import calculate_time, get_guild_icon
...
class SetupCommands(commands.Cog, name='Setup'):
    ...
    async def link_parse(
        ...
        embed = disnake.Embed(
            title='Link Parse Settings Updated',
            description=text,
    ...
    async def addClan(
        ...
        if member_role.is_bot_managed():
            embed = disnake.Embed(
                description='Clan Roles cannot be bot roles.',
                color=disnake.Color.red(),
            )
            return await ctx.edit_original_message(embed=embed)
