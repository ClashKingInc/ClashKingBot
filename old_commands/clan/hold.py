...
async def clan_capital_overview(bot: CustomClient, clan: coc.Clan, weekend: str, embed_color: disnake.Color):
    ...
    attack_summary_text = '```a=atks, d=districts, l=loot\n'
    for raid_clan in raid_log_entry.attack_log:
    ...
    attack_summary_text += '```'
    defense_summary_text += '```'
    embed.add_field(name='Attack Summary', value=attack_summary_text, inline=False)
...
async def create_offensive_hitrate(
    ...
    embed = disnake.Embed(title='Offensive Hit Rates', description=text, colour=disnake.Color.green())
    filter_types = []
...
async def create_defensive_hitrate(
    ...
    if len(ranked) == 0:
        text = 'No War Attacks Tracked Yet.'
    embed = disnake.Embed(title='Defensive Rates', description=text, colour=disnake.Color.green())
    filter_types = []
    if True in fresh_type:
        filter_types.append('Fresh')
    if False in fresh_type:
...
async def create_stars_leaderboard(
    ...
    if len(ranked) == 0:
        text = 'No War Attacks Tracked Yet.'
    embed = disnake.Embed(title='Star Leaderboard', description=text, colour=disnake.Color.green())
    filter_types = []
    if True in fresh_type:
        filter_types.append('Fresh')
    if False in fresh_type:
