...
class RosterCommands(commands.Cog, name='Rosters'):
    ...
    async def roster_refresh(
        ...
        if roster != 'REFRESH ALL':
            ...
            await ctx.edit_original_message('Bulk Roster Refresh Has Been Added to Queue')
    ...
    async def roster_ping(
        ...
        top_text = (
            ...
            f"{_roster.clan_name} | {_roster.alias} | {'' if not _roster.time else self.bot.timestamp(_roster.time).relative}\n"
        )
    ...
    async def roster_restrict(
        ...
        await self.bot.rosters.update_one(
            {'$and': [{'server_id': ctx.guild.id}, {'alias': roster}]},
            {'$set': {'token': url_safe_uuid}},
        )
        await ctx.send(
            content=f'Edit your roster here -> https://api.clashk.ing/roster/?token={url_safe_uuid}',
            ephemeral=True,
        )
    ...
    async def roster_list(self, ctx: disnake.ApplicationCommandInteraction):
        ...
        for roster in await result.to_list(length=count):
            ...
            if _roster.time is not None:
                text += f' | {self.bot.timestamp(_roster.time).cal_date}\n'
            else:
                text += '\n'
    ...
    async def roster_role_refresh(
        ...
        if roster != 'REFRESH ALL':
            ...
            await ctx.edit_original_message('Bulk Roster Role Refresh Has Been Added to Queue')
    ...
    async def on_button_click(self, ctx: disnake.MessageInteraction):
        if 'Refresh_' in str(ctx.data.custom_id):
            ...
            url_safe_uuid = base64_uuid.decode('utf-8')
            await self.bot.rosters.update_one(
                {'$and': [{'server_id': ctx.guild.id}, {'alias': roster.alias}]},
                {'$set': {'token': url_safe_uuid}},
            )
            await ctx.send(
                content=f'Edit your roster here -> https://api.clashk.ing/roster/?token={url_safe_uuid}',
            ...
            button_options = [disnake.SelectOption(label='Main Group', value='signup_No Group')]
            ...
            msg = await ctx.followup.send(
                content='Select Account(s) to Add & Where to add them',
                components=dropdown,
                ephemeral=True,
            )
