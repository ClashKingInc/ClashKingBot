...
async def legend_day_overview(
    ...
    for hit in legend_day.attacks:
        if hit.timestamp is None:
            ...
            for gear in hit.hero_gear:
                gears_used.add(gear)
            off += f'{bot.emoji.clash_sword} +{hit.change} {bot.timestamp(hit.timestamp).relative}\n'
    ...
    for d in legend_day.defenses:
        defi += f'{bot.emoji.shield.emoji_string} -{d.change} {bot.timestamp(d.timestamp).relative}\n'
