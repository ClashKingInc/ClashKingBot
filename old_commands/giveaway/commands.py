...
class GiveawayCommands(commands.Cog):
    ...
    @giveaway.sub_command(
        name='reroll',
        description='Reroll one or more winners of a giveaway using mentions.',
    )
    @commands.check_any(commands.has_permissions(manage_guild=True), check_commands())
    async def giveaway_reroll(
        ...
        if invalid_users:
            ...
            await ctx.send(
                f'❌ The following users are not current winners of this giveaway: {invalid_mentions}',
                ephemeral=True,
            )
            return
        ...
        if len(eligible_participants) < len(user_ids_to_replace):
            await ctx.send(
                f'❌ Not enough eligible participants to replace {len(user_ids_to_replace)} users.',
                ephemeral=True,
            )
            return
        ...
        if not channel:
            await ctx.send(
                '❌ Could not find the channel associated with the giveaway.',
                ephemeral=True,
            )
            return
        ...
        await ctx.send(
            f'✅ Reroll completed. New winners have been announced in <#{channel_id}>.',
            ephemeral=True,
        )
