...
from utility.emojis import create_superscript, get_guild_icon, smart_convert_seconds
...
async def family_overview(bot: CustomClient, server: disnake.Guild, embed_color: disnake.Color):
    ...
    description_text = (
        ...
        f'- Created: {bot.timestamp(unix_time=int(server.created_at.timestamp())).text_date}\n'
...
async def family_raids(bot: CustomClient, server: disnake.Guild, weekend: str, embed_color: disnake.Color):
    ...
    embed = disnake.Embed(description='**Current Raids**', color=embed_color)
...
async def family_wars(bot: CustomClient, server: disnake.Guild, embed_color: disnake.Color):
    ...
    embed = disnake.Embed(description='**Current Wars**', color=embed_color)
    for war in war_list:
        ...
        embed.add_field(
            ...
            f'> {war_pos}: {bot.timestamp(int(war_time)).relative}',
            inline=False,
        )
