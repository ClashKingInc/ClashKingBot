...
class FamilyStats(commands.Cog, name="Family Trophy Stats"):
    ...
    async def player_rank(self, ctx: disnake.ApplicationCommandInteraction, user: disnake.User):
        ...
        for player in players:
            ...
            embed = disnake.Embed(description=f"Name: {player.name}\n" +
                                    f"Tag: {player.tag}\n" +
                                    f"Clan: {clan}\n" +
        ...
        while True:
            ...
            if res.data.custom_id == "Previous":
                current_page -= 1
                await res.response.edit_message(embed=embeds[current_page], components=create_components(current_page, embeds))
    ...
    @ranked.sub_command(name="best", description="Arranges players to create best 5 clans if eos was now")
