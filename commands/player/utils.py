...
from utility.clash.other import heros, heroPets, profileSuperTroops
...
async def lookup_components(bot: CustomClient, ctx: disnake.MessageInteraction, custom_player: StatsPlayer, **kwargs):
...
async def player_todo_settings(bot: CustomClient, ctx: disnake.MessageInteraction):
    ...
    def player_component(bot: CustomClient, all_players: List[coc.Player]):
        ...
        for chunk_list in player_chunked:
            ...
            for player in chunk_list:
                player_options.append(disnake.SelectOption(label=f"{player.name} ({player.tag})",
        ...
        dropdown.append(
            disnake.ui.ActionRow(disnake.ui.Button(label="Save", emoji=bot.emoji.yes.partial_emoji, style=disnake.ButtonStyle.green, custom_id="Save"))
        )
        return dropdown

    msg = await ctx.followup.send(content="Choose which account you want visible on your to-do list (max 15)\n",
                                    components=player_component(bot=bot, all_players=user_accounts), ephemeral=True, wait=True)
    clicked_save = False
    players = set()
    ...
    await bot.user_settings.update_one({"discord_id" : ctx.author.id}, {"$set" : {"to_do_accounts" : list(players)}}, upsert=True)
