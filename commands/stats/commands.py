...
class StatCommands(commands.Cog, name="Stat Commands"):
    ...
    async def war(self, ctx: disnake.ApplicationCommandInteraction,
                        ...
                        star_filter: str = commands.Param(default="3", description="Comma seperated list (default: '3', example: '3, 2, 1')"),
        ...
        if type == "Hit Rate":
            embed = await war_hitrate(bot=self.bot, clan=clan, server=server, th_filter=th_filter.replace(" ", "").lower(),
                                      star_filter=star_filter, limit=limit, min_attacks=min_attacks,
                                      war_types=war_types.replace(" ", "").lower(), season=season,
                                      num_wars=num_wars, num_days=num_days, embed_color=embed_color)
