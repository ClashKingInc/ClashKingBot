...
async def main_war_page(bot: CustomClient, war: coc.ClanWar, war_league=None, is_previous=False):
    ...
    if war.type == "cwl":
        ...
        state_text = (
            f"{cwl_league_emojis(bot=bot, league=str(war_league))}{str(war_league)}\n" + state_text)
...
async def attacks_embed(bot: CustomClient, war: coc.ClanWar):
    ...
    if missing_attacks:
        split = [missing_attacks[i:i + 20]
                 for i in range(0, len(missing_attacks), 20)]
        for item in split:
            embed.add_field(name="**No attacks done:**",
                            value="".join(item), inline=False)
    embed.set_thumbnail(url=war.clan.badge.large)
    return embed
...
async def defenses_embed(bot: CustomClient, war: coc.ClanWar):
    ...
    if missing_defenses:
        split = [missing_defenses[i:i + 20]
                 for i in range(0, len(missing_defenses), 20)]
        for item in split:
...
title += ") **"'''
        title = "**"
        title += ",".join(
            [f"{plan.name}{create_superscript(num=plan.townhall_level)}" for plan in plan_list])
        title += "**\n"
...
async def missed_hits(bot: CustomClient, war: coc.ClanWar):
    ...
    if two_hit_missed:
        embed.add_field(name="Two Hits Missed",
                        value="\n".join(two_hit_missed))
...
async def get_cwl_wars(bot: CustomClient, clan: coc.Clan, season: str, group=None, fetched_clan=None):
    ...
    if not clan_league_wars:
        ...
        if group_response is not None:
            ...
            league_name = (basic_clan.get("changes", {}).get("clanWarLeague", {}).get(
                get_previous_month(season), {}).get("league", "Unranked"))
...
async def star_lb(league_wars, clan, defense=False):
    ...
    sorted_list = sorted(
        star_list, key=operator.itemgetter(1, 2), reverse=True)
    text = ""
...
async def ranking_lb(bot: CustomClient, group: coc.ClanWarLeagueGroup, fetched_clan: str = None):
    ...
    if fetched_clan is not None:
        ...
        league_wars = wars_from_group(
            bot=bot, data=clan_league_wars, group=group)
    ...
    for round in group.rounds:
        for war_tag in round:
            if not league_wars:
                ...
                war = get_league_war_by_tag(
                    league_wars=league_wars, war_tag=war_tag)
            if str(war.status) == "won":
    ...
    embed = disnake.Embed(title="Clan Ranking Leaderboard",
                          description=text, color=disnake.Color.green())
    embed.set_footer(
        text=f"Avg {avg_stars_per_war:.1f} Stars/War | Avg Win {avg_win_war:.1f} Stars")
    return embed
...
async def overall_stat_components(bot: CustomClient):
    ...
    options = [  # the options in your dropdown
        ...
        disnake.SelectOption(label="Missed Hits",
                             ...
                             value="missedhits"),
        disnake.SelectOption(
            label="All Rounds", emoji=bot.emoji.wrench.partial_emoji, value="allrounds"),
        disnake.SelectOption(
            label="All Members",
...
async def stat_components(bot: CustomClient, war: coc.ClanWar, next_war: coc.ClanWar):
    ...
    if war is None:
        ...
        options.insert(
            1,
            disnake.SelectOption(label="Next Round Lineup",
                                 emoji=troop.partial_emoji, value="lineup"),
        )
    elif next_war is None:
        options.insert(0, disnake.SelectOption(
            label="Current Round", emoji=swords.partial_emoji, value="round"))
        options.insert(
            1,
            disnake.SelectOption(
                label="Current Lineup", emoji=troop.partial_emoji, value="current_lineup"),
        )
        options.insert(
            2,
            disnake.SelectOption(
...
async def round_components(league_wars: List[coc.ClanWar]):
    options = [disnake.SelectOption(
        label=f"Overview", value=f"cwlround_overview")]
    for round in range(1, len(league_wars) + 1):
        options.append(disnake.SelectOption(
            label=f"Round {round}", value=f"cwlround_{round}"))
...
async def clan_components(group: coc.ClanWarLeagueGroup):
    ...
    for clan in group.clans:
        options.append(disnake.SelectOption(
            label=f"{clan.name}", value=f"cwlchoose_{clan.tag}"))
...
def wars_from_group(bot: CustomClient, group: coc.ClanWarLeagueGroup, data: list[dict], clan_tag=None):
    list_wars = []
    for war in data:
        war = coc.ClanWar(data=war.get("data"),
                          client=bot.coc_client, league_group=group)
...
async def cwl_ranking_create(bot: CustomClient, clan: coc.Clan):
    ...
    place = 1
    for item in sorted_list:
        promo = [x["promo"] for x in war_leagues["items"]
                 if x["name"] == clan.war_league.name][0]
        demo = [x["demote"] for x in war_leagues["items"]
                if x["name"] == clan.war_league.name][0]
        if place <= promo:
