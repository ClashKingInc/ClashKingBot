...
class ClanCommands(commands.Cog, name="Clan Commands"):
    ...
    async def progress(self, ctx: disnake.ApplicationCommandInteraction,
        ...
        if type == "Heroes & Pets":
            ...
            embeds = await clan_hero_progress(bot=self.bot, clan=clan, season=season, limit=limit, embed_color=embed_color)


        elif type == "Troops, Spells, & Sieges":
            custom_id = f"clantroops:{clan.tag}:{season}:{limit}"
            embeds = await troops_spell_siege_progress(bot=self.bot, clan=clan, season=season, limit=limit, embed_color=embed_color)
    ...
    async def sorted(self, ctx: disnake.ApplicationCommandInteraction,
        ...
        """
        embed_color = await self.bot.ck_client.get_server_embed_color(server_id=ctx.guild_id)
        embed = await clan_sorted(bot=self.bot, clan=clan, sort_by=sort_by, limit=limit, townhall=townhall, embed_color=embed_color)
    ...
    async def donations(self, ctx: disnake.ApplicationCommandInteraction,
        ...
        embed_color = await self.bot.ck_client.get_server_embed_color(server_id=ctx.guild_id)
        embed = await clan_donations(bot=self.bot, clan=clan, season=season, townhall=townhall, limit=limit, sort_by=sort_by, sort_order=sort_order, embed_color=embed_color)
        buttons = disnake.ui.ActionRow()
        buttons.append_item(disnake.ui.Button(
            label="", emoji=self.bot.emoji.refresh.partial_emoji,
            style=disnake.ButtonStyle.grey,
    ...
    async def war_preference(self, ctx: disnake.ApplicationCommandInteraction,
                                  clan: coc.Clan = options.clan,
                                  option: str = commands.Param(default="Last Opt Change", choices=["Last Opt Change", "Last War", "War Timer"])):
        embed_color = await self.bot.ck_client.get_server_embed_color(server_id=ctx.guild_id)
        embeds = await clan_warpreference(bot=self.bot, clan=clan, option=option.lower().replace(" ", ""), embed_color=embed_color)
        buttons = disnake.ui.ActionRow()
        buttons.add_button(
            label="", emoji=self.bot.emoji.opt_in.partial_emoji,
            style=disnake.ButtonStyle.grey,
            custom_id=f"clanwarpref:{clan.tag}:lastoptchange")
        buttons.add_button(
            label="", emoji=self.bot.emoji.wood_swords.partial_emoji,
            style=disnake.ButtonStyle.grey,
            custom_id=f"clanwarpref:{clan.tag}:lastwar")
        buttons.add_button(
            label="", emoji=self.bot.emoji.clock.partial_emoji,
            style=disnake.ButtonStyle.grey,
            custom_id=f"clanwarpref:{clan.tag}:wartimer")
        await ctx.edit_original_message(embeds=embeds, components=[buttons])


    @clan.sub_command(name="activity", description="Activity related stats for a clan - last online, activity")
    async def activity(self, ctx: disnake.ApplicationCommandInteraction,
        ...
        embed_color = await self.bot.ck_client.get_server_embed_color(server_id=ctx.guild_id)
        embed = await clan_activity(bot=self.bot, clan=clan, townhall=townhall, limit=limit, season=season,
    ...
    async def capital(self, ctx: disnake.ApplicationCommandInteraction,
                           clan: coc.Clan = options.clan,
