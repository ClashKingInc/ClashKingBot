...
from utility.discord_utils import  get_webhook_for_channel, check_commands, interaction_handler
...
class MessageCommands(commands.Cog):
    ...
    async def refresh_board(self, ctx: disnake.MessageCommandInteraction, message: disnake.Message):
        ...
        if len(options) >= 2:
            ...
            await ctx.edit_original_message(content="Choose which button to make an auto refreshing board for", components=[disnake.ui.ActionRow(option_select)])
        ...
        placeholder = disnake.Embed(description="Placeholder Embed, will update momentarily!", color=disnake.Color.green())
