...
class Suggestions(commands.Cog):
    ...
    @commands.slash_command(name="suggest", description="Suggest a feature for the bot", guild_ids=[923764211845312533])
    async def suggest(self, ctx: disnake.ApplicationCommandInteraction, topic: str, description: str):
        ...
        embed = disnake.Embed(
            title=topic,
            description=description,
            color=0x000000  # Starting as black
        )
        embed.set_footer(text=f"Suggestion ID: {str(suggestion_id)}")
        message = await channel.send(
            embed=embed,
            components=[
                disnake.ui.Button(style=disnake.ButtonStyle.green, label="Upvote", custom_id=f"upvote_{suggestion_id}"),
            ]
        )
    ...
    @commands.is_owner()
    async def remove_suggestion(self, ctx: disnake.ApplicationCommandInteraction, suggestion_id: str = commands.Param(description="The ID of the suggestion to remove")):
    ...
    @commands.is_owner()
    async def ping_upvoters(self, inter: disnake.ApplicationCommandInteraction, suggestion_id: str = commands.Param(description="The ID of the suggestion")):
    ...
    async def on_button_click(self, ctx: disnake.MessageInteraction):
        ...
        if custom_id.startswith("upvote_"):
            ...
            if user_id in suggestion['voters']:
                ...
                return
            await self.bot.suggestions.update_one({"_id": suggestion_id}, {"$inc" : {"votes" : 1}, "$push": {"voters": user_id}})
