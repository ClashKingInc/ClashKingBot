...
async def daily_graph(bot: CustomClient, clan_tags: List[str], attribute: str, months: int, limit: int = 20, html: bool = False) -> (disnake.File, str):
    ...
    if attribute == "activity":
        ...
        pipeline = [{"$match" : {"$and" : [{"tag" : {"$in" : list(member_map.keys())}}, {"type" : {"$in" : lookup}}, {"time" : {"$gte" : START_TIME}}, {"time" : {"$lte" : END_TIME}}]}},
...
async def season_line_graph(bot: CustomClient, clan_tags: List[str], attribute: str, months: int, limit: int = 20, html: bool = False) -> (disnake.File, str):
    clan_name_map = await bot.get_clan_name_mapping(clans=clan_tags)
    ...
    if attribute in ["activity", "clan_games", "attack_wins", "donations", "received"]:
        ...
        countable = False
        basic_clan = await bot.basic_clan.find({"tag": {"$in": clan_tags}}, projection={"_id" : 0, "tag" : 1, "changes.clanWarLeague" : 1}).sort("members", -1).limit(limit).to_list(length=None)
        data = defaultdict(lambda : defaultdict(int))
...
async def monthly_bar_graph(bot: CustomClient, clan_tags: List[str], attribute: str, season: str, html: bool = False) -> (disnake.File, str):
    ...
    if attribute in ["gold_looted", "elixir_looted", "de_looted"]:
        ...
        player_stats = await bot.player_stats.find({"tag" : {"$in" : member_tags}}, projection={"_id" : 0, f"gold.{season}" : 1, f"elixir.{season}" : 1, f"dark_elixir.{season}" : 1, "tag" : 1}).to_list(length=None)
        attribute_map = {"gold_looted": "gold", "elixir_looted": "elixir", "de_looted": "dark_elixir"}
        ...
        attribute_map = {"donations": "donated", "received": "received", "activity": "activity", "attack_wins" : "attack_wins"}
        data = defaultdict(int)
        ...
        data = defaultdict(int)
        player_stats = await bot.player_stats.find({"tag": {"$in": member_tags}}, projection={"_id": 0, f"capital_gold": 1, "tag": 1}).to_list(length=None)
        ...
        raid_stats = await bot.raid_weekend_db.find({"$and" : [{"clan_tag" : {"$in" : clan_tags}}, {"data.startTime" : {"$in" : times_to_pull}}]},
                                                    projection={"clan_tag" : 1, "_id" : 0, "data.capitalTotalLoot" : 1}).to_list(length=None)
        ...
        for week in season_raid_weeks:
            ...
            times_to_pull.append(weekend_to_iso.strftime('%Y%m%dT%H%M%S.000Z'))
        raid_stats = await bot.raid_weekend_db.find({"$and" : [{"clan_tag" : {"$in" : clan_tags}}, {"data.startTime" : {"$in" : times_to_pull}}]},
                                                    projection={"clan_tag" : 1, "_id" : 0, "data.members" : 1}).to_list(length=None)
        holder = defaultdict(list)
        for raid in raid_stats:
            clan_name = name_mapping.get(raid.get("clan_tag"))
        ...
        enum_map = {"troops_upgraded" : coc.enums.HOME_TROOP_ORDER + coc.enums.SPELL_ORDER + coc.enums.BUILDER_TROOPS_ORDER,
