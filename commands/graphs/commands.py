...
class GraphCreator(commands.Cog, name="Graph Commands"):
    ...
    async def graph_over_time(self, ctx: disnake.ApplicationCommandInteraction,
        ...
        graph, web = await season_line_graph(clan_tags=clans, months=months, bot=self.bot, attribute=attribute.lower().replace(' ', "_"), limit=limit, html=True)
    ...
    async def graph_over_month(self, ctx: disnake.ApplicationCommandInteraction,
                              attribute: str = commands.Param(choices=["Gold Looted", "Elixir Looted", "DE Looted",
        ...
        graph, web = await monthly_bar_graph(bot=self.bot, clan_tags=clan_tags, attribute=attribute.lower().replace(" ", "_"), season=season, html=True)
        buttons = disnake.ui.ActionRow()
        buttons.append_item(
            disnake.ui.Button(style=disnake.ButtonStyle.url, label="Online View", url=web))
        await ctx.send(file=graph, components=[buttons])
    ...
    async def graph_daily(self, ctx: disnake.ApplicationCommandInteraction,
        ...
        if clans is None:
            ...
            clans = [clan.split("|")[-1] if "|" in clan else clan for clan in clans]
        graph, web = await daily_graph(bot=self.bot, clan_tags=clans, months=months, attribute=attribute.replace(" ", ""), limit=limit, html=True)
        buttons = disnake.ui.ActionRow()
        buttons.append_item(
            disnake.ui.Button(style=disnake.ButtonStyle.url, label="Online View", url=web))
        await ctx.send(file=graph, components=[buttons])
