...
class FamilyCommands(commands.Cog, name="Family Commands"):
    ...
    async def compo(
        ...
        embed = await family_composition(
            bot=self.bot, server=server, type=type_, embed_color=embed_color
        )
    ...
    @family.sub_command(
        name="clans",
        description="Boards showing overview of the family clans in different ways",
    )
    async def clans(
        self,
        ctx: disnake.ApplicationCommandInteraction,
        ...
        if types == "Overview":
            embed = await family_overview(
                bot=self.bot, server=server, embed_color=embed_color
            )
            button_id = f"familyoverview:{server.id}"
        elif types == "Categories":
            embed = await family_clans(
                bot=self.bot, server=server, type="db", embed_color=embed_color
            )
            button_id = f"familyclans:{server.id}:db"
        elif types == "CWL Leagues":
            embed = await family_clans(
                bot=self.bot, server=server, type="cwl", embed_color=embed_color
            )
            button_id = f"familyclans:{server.id}:cwl"
        elif types == "Capital Leagues":
            embed = await family_clans(
                bot=self.bot, server=server, type="capital", embed_color=embed_color
            )
            button_id = f"familyclans:{server.id}:capital"
        elif types == "Location":
            embed = await family_clans(
                bot=self.bot, server=server, type="location", embed_color=embed_color
            )
            button_id = f"familyclans:{server.id}:location"
        elif types == "TH Requirement":
            embed = await family_clans(
                bot=self.bot, server=server, type="th", embed_color=embed_color
            )
    ...
    async def raids(
        ...
        embed = await family_raids(
            bot=self.bot, server=server, weekend=None, embed_color=embed_color
        )
    ...
    @family.sub_command(
        name="progress", description="Progress in various areas for the family"
    )
    async def progress(
        self,
        ctx: disnake.ApplicationCommandInteraction,
        ...
        if type == "Heroes & Pets":
            ...
            embeds = await family_hero_progress(
                bot=self.bot,
                server=server,
                season=season,
                limit=limit,
            ...
            embeds = await family_troop_progress(
                bot=self.bot,
                server=server,
                season=season,
                limit=limit,
    ...
    @family.sub_command(
        name="sorted", description="List of family members, sorted by any attribute"
    )
    async def sorted(
        self,
        ctx: disnake.ApplicationCommandInteraction,
    ...
    async def donations(
        ...
        sort_by: str = commands.Param(
            default="Donations", choices=["Donations", "Received"]
        ),
    ...
    @family.sub_command(
        name="activity",
        description="Activity related stats for a family - last online, activity",
    )
    async def activity(
        self,
        ctx: disnake.ApplicationCommandInteraction,
        ...
        limit: int = commands.Param(default=50, min_value=1, max_value=50),
        sort_by: str = commands.Param(
            default="Activity", choices=["Activity", "Last Online"]
        ),
        sort_order: str = commands.Param(
            default="Descending", choices=["Ascending", "Descending"]
        ),
    ...
    async def image(
        ...
        board: str = commands.Param(
            choices=["Activity", "Legends", "Trophies", "War Stars"]
        ),
        ...
        if clan is None:
            server = server or ctx.guild
        if board == "Legends":
            file = await image_board(
                bot=self.bot, clan=clan, server=server, type="legend", limit=limit
            )
