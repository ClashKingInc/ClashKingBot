...
class Trials(commands.Cog, name="Trials"):
    ...
    async def trial_create(self, ctx: disnake.ApplicationCommandInteraction, channel: disnake.TextChannel, player: coc.Player = commands.Param(converter=player_converter),
                           duration: int = commands.Param(le=35), trophies: int = None, attack_wins: int = None, combined_hero_level: int = None, hitrate: int = None,
                           capital_gold_looted: int = None, capital_gold_donated: int = None, allowed_clan_hops: int = None, num_wars_participated: int = None):
        result =await self.bot.trials.find_one({"$and" : [{"player_tag" : player.tag}, {"server_id" : ctx.guild_id}, {"active" : True}]})
        ...
        text = ""
        items = [trophies, attack_wins, combined_hero_level, hitrate, capital_gold_looted, capital_gold_donated, allowed_clan_hops, num_wars_participated]
        item_text = ["Trophies", "Attack Wins", "Combined Hero Level", "Hitrate", "Capital Gold Looted", "Capital Gold Donated", "Allowed Clan Hops", "\# Wars Participated"]
        for item, text in zip(items, item_text):
            if item is not None:
                text += f"- **{text}:** {item}\n"

        embed = disnake.Embed(title=f"{duration} day Trial for {player.name}",
    ...
    @commands.check_any(commands.has_permissions(manage_guild=True), check_commands())
    async def trial_notate(self, ctx: disnake.ApplicationCommandInteraction, player: coc.Player = commands.Param(converter=player_converter)):
        pass


    @trial.sub_command(name='edit', description="Edit a trial for a player")
    @commands.check_any(commands.has_permissions(manage_guild=True), check_commands())
    async def trial_edit(self, ctx: disnake.ApplicationCommandInteraction, player: coc.Player = commands.Param(converter=player_converter)):
        pass
