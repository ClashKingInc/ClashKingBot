...
from .utils import hv_player_leaderboard, hv_clan_leaderboard, clan_capital_leaderboard, bb_clan_leaderboard, bb_player_leaderboard
...
class Leaderboards(commands.Cog, name='Leaderboards'):
    ...
    async def leaderboard_location(self, ctx: disnake.ApplicationCommandInteraction,
        ...
        if country == "Global":
            country_id = "global"
            location_emoji = self.bot.emoji.globe.emoji_string
        else:
        ...
        if type == "HV Players":
            embeds = await hv_player_leaderboard(bot=self.bot, country_name=country.capitalize(), rankings=rankings, location_emoji=location_emoji, embed_color=embed_color)
        elif type == "HV Clans":
            embeds = await hv_clan_leaderboard(bot=self.bot, country_name=country.capitalize(), rankings=rankings,
                                                 location_emoji=location_emoji, family_clan_tags=family_clan_tags, embed_color=embed_color)
        elif type == "Clan Capital":
            embeds = await clan_capital_leaderboard(bot=self.bot, country_name=country.capitalize(), rankings=rankings,
                                               location_emoji=location_emoji, family_clan_tags=family_clan_tags, embed_color=embed_color)
        elif type == "BB Players":
            embeds = await bb_player_leaderboard(bot=self.bot, country_name=country.capitalize(), rankings=rankings,
                                               location_emoji=location_emoji, family_clan_tags=family_clan_tags, embed_color=embed_color)
        elif type == "BB Clans":
            embeds = await bb_clan_leaderboard(bot=self.bot, country_name=country.capitalize(), rankings=rankings,
                                               location_emoji=location_emoji, family_clan_tags=family_clan_tags, embed_color=embed_color)
