    ...
    if players == clans == server == None:
        ...
        for member in member_data:
            if not tied_donations_only:
                player_struct[member.get("tag")]["donations"] = member.get("donations", {}).get(season, {}).get("donations", 0)
...
async def activity(request: Request, response: Response,
                           player: Annotated[List[str], Query(max_length=50)]=None,
                           clan: Annotated[List[str], Query(max_length=25)]=None,
                           server: Annotated[List[str], Query(max_length=5)]=None):
    return {}
