...
class getFamily(commands.Cog):
    ...
    async def create_donations(self, guild: disnake.Guild, type: str):
        ...
        for player in responses:
            ...
            name = name[:13]
            name = name.ljust(13)
            if player.clan.tag not in badges:
                badge = await self.bot.create_new_badge_emoji(url=player.clan_badge_link())
                badges[player.clan.tag] = badge
            else:
                badge = badges[player.clan.tag]
            donated_text.append(
                [f"{badge}` {str(player.donos.donated).ljust(5)} | {str(player.donos.received).ljust(5)} | \u200e{name}`",
                 player.donos.donated])
            received_text.append(
                [f"{badge}` {str(player.donos.received).ljust(5)} | {str(player.donos.donated).ljust(5)} | \u200e{name}`",
                 player.donos.received])
            ratio_text.append([f"{badge}` {str(player.donation_ratio).ljust(5)} | \u200e{name}`", player.donation_ratio])
