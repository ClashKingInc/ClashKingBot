...
import disnake
from CommandsOld.board_generation import activity_board, donation_board, games_board
...
async def basic_parser(bot: CustomClient, result: dict):
    ...
    title, icon = await bot.parse_to_name_icon(
        as_dict={
            'user': result.get('user'),
            'family': result.get('server'),
            'clan': result.get('clans'),
        }
    )
    ...
    if type == 'donations':
        ...
        embed = donation_board(
            bot=bot,
            result=result,
            title_name=title,
            footer_icon=icon,
            embed_color=disnake.Color(server_result.get('embed_color', 0x2ECC71)),
        )
    elif type == 'activity':
        result = await bot.ck_client.get_activity(as_dict=result)
        embed = activity_board(
            bot=bot,
            result=result,
            title_name=title,
            footer_icon=icon,
