...
class TopCommands(commands.Cog, name="Top"):
    ...
    async def donations(
        ...
        buttons.append_item(
            disnake.ui.Button(
                ...
                style=disnake.ButtonStyle.grey,
                custom_id=f"topreceivedplayer_",
            )
        )
    ...
    async def family_compo(
        ...
        if type == "Totals":
            embed = await shared_embeds.th_composition(
                ...
                title=f"Bot Townhall Composition",
    ...
    async def activities(
        ...
        buttons.append_item(
            disnake.ui.Button(
                ...
                emoji=self.bot.emoji.refresh.partial_emoji,
                style=disnake.ButtonStyle.grey,
                custom_id=f"topactivityplayer_{season}",
            )
        )
    ...
    async def sorted(
        ...
        embed = await shared_embeds.player_sort(
            bot=self.bot,
            player_tags=[],
            sort_by=sort_by,
            footer_icon=self.bot.user.avatar.url,
            title_name=f"All Players sorted by {sort_by}",
            limit=limit,
        )
    ...
    async def capital(self, ctx: disnake.ApplicationCommandInteraction, weekend: str = None):
        ...
        buttons.append_item(
            disnake.ui.Button(
                ...
                style=disnake.ButtonStyle.grey,
                custom_id=f"topcapitaldonatedplayer_{weekend}",
            )
        )
        buttons.append_item(
            disnake.ui.Button(
                ...
                style=disnake.ButtonStyle.grey,
                custom_id=f"topcapitalraidplayer_{weekend}",
            )
        )
