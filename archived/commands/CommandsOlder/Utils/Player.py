...
import coc
import disnake
from coc.raid import RaidLogEntry, RaidMember
...
async def cwl_stalk(
	...
	for player in tags:
		results = (await bot.warhits.find({
		    "$and": [
		        {
		            "tag": player
		        },
	...
	last_30_days = (
	    await bot.warhits.find({
	        "$and": [
	            ...
	            {
	                "_time": {
	                    "$gte": int((datetime.now() - timedelta(days=35)).timestamp())
	                }
	            },
	        ]
	...
	buttons = []
	if len(embeds) > 4:
		buttons = [disnake.ui.ActionRow(disnake.ui.Button(
