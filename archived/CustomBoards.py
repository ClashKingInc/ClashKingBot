...
from utility.discord_utils import permanent_image, interaction_handler
...
class CustomBoards(commands.Cog):
    ...
    async def create(
        ...
        alias: str = commands.Param(min_length=3, max_length=25),
        embed_link: str = None,
    ):
            ...
            try:
                ...
                if channel is None:
                    return await ctx.send(
                        content=f"Cannot access the channel this embed is in",
                        ephemeral=True,
                    )
        ...
        while True:
            ...
            if str(res.data.custom_id) == "add_button":
                ...
                button_select = disnake.ui.StringSelect(
                    ...
                    placeholder="Choose a button type",
                    max_values=1,
                )
                msg = await res.followup.send(
                    content="Select Button Type",
