...
def gen_season_start_end_as_iso(season: str):
    year = season[:4]
    month = season[-2:]
    SEASON_START = coc.utils.get_season_start(month=int(month), year=int(year)).timestamp()
    SEASON_END = coc.utils.get_season_end(month=int(month), year=int(year)).timestamp()
