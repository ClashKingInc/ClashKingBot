...
async def get_keys(emails: list, passwords: list, key_names: str, key_count: int):
    ...
    for count, email in enumerate(emails):
        ...
        if len(keys) == 10 and len(_keys) < key_count:
            logger.info("%s keys were requested to be used, but a maximum of %s could be "
        ...
        await session.close()
        #logger.info("Successfully initialised keys for use.")
        for k in _keys:
            total_keys.append(k)

    logger.info(len(total_keys))
    return (total_keys)
...
async def main():
    ...
    while True:
        try:
            ...
            logger.info(f"{len(all_tags_to_track)} tags")
                    ...
                    async def fetch(url, session: aiohttp.ClientSession, headers):
                        async with session.get(url, headers=headers) as new_response:
                            ...
                            if new_response != previous_response:
                                new_compressed_response = snappy.compress(new_response)
                                await cache.set(obj.tag, new_compressed_response, ex=2_592_000)
                                    ...
                                    for (parent, type_), (old_value, value) in changes.items():
                                        ...
                                        if type_ == "donations":
                                            ...
                                            previous_dono = 0 if (previous_dono := previous_response["donationsReceived"]) > (
                                                current_dono := new_response["donationsReceived"]) else previous_dono
                                            ...
                                            player_level_changes.update({"$push": {
                                                f"capital_gold.{raid_date}.donate": (new_response["clanCapitalContributions"] - previous_response["clanCapitalContributions"])}})
                                            clan_level_changes.update({"$inc": {
                                                f"{season}.{tag}.capital_gold_dono": (new_response["clanCapitalContributions"] - previous_response["clanCapitalContributions"])}})
            ...
            logger.info(f"{time.time() - time_inside} seconds inside")
            logger.info(f"{len(players_tracked)} players tracked")
