...
class RefreshBoards(commands.Cog):
    def __init__(self, bot: CustomClient):
        self.bot = bot
        self.bot.scheduler.add_job(self.refresh, 'interval', minutes=1, next_run_time=pend.now(tz=pend.UTC).add(seconds=10))
        self.position_check: bool = False
        self.bot.scheduler.add_job(self.set_check_state, 'interval', hours=12, next_run_time=pend.now(tz=pend.UTC).add(minutes=1))

    async def set_check_state(self):
        self.position_check = True

    async def refresh(self):
        all_refresh_boards = await self.bot.autoboards.find({'webhook_id': {'$ne': None}}).to_list(length=None)
        for board in all_refresh_boards:
            ...
            '''if board.get('server') not in self.bot.OUR_GUILDS:
                continue'''
