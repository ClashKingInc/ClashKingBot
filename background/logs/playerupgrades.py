    ...
    async def league_change(self, event):
        ...
        for cc in await self.bot.clan_db.find(
            ...
            if clan.server_id not in self.bot.OUR_GUILDS:
                continue
            content = f'{self.bot.fetch_emoji(name=new_player.town_hall)}[{name}](<{new_player.share_link}>) moved from {league_emoji(bot=self.bot, player=old_player)}{old_player.league.name} to {league_emoji(bot=self.bot, player=new_player)}{new_player.league.name}'
