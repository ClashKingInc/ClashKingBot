...
class War_Log(commands.Cog):
    ...
    async def war_attacks(self, event):
        ...
        for attack in attacks:
            ...
            if attack.attacker.clan.tag == war.clan.tag:
                hit_text += f"{self.bot.emoji.thick_sword} {self.bot.fetch_emoji(attack.attacker.town_hall)}**{attack.attacker.name}{create_superscript(num=attack.attacker.map_position)}**" \
                ...
                # is a defense
                hit_text += f"{self.bot.emoji.shield} {self.bot.fetch_emoji(attack.defender.town_hall)}**{attack.defender.name}{create_superscript(num=attack.defender.map_position)}**" \
    ...
    async def war_state_change(self, event):
        ...
        if cwl_group:
            new_war = coc.ClanWar(data=event["new_war"], client=self.bot.coc_client, league_group=coc.ClanWarLeagueGroup(data=cwl_group, client=self.bot.coc_client), clan_tag=event.get("clan_tag"))
        else:
            new_war = coc.ClanWar(data=event["new_war"], client=self.bot.coc_client, clan_tag=event.get("clan_tag"))
