...
class Bans(commands.Cog, name="Bans"):
    ...
    async def ban_add(self, ctx: disnake.ApplicationCommandInteraction,
                      player_tag: str = commands.Param(autocomplete=autocomplete.family_players),
                      reason: str = "No Notes",
                      rollover_days: int = commands.Param(name="rollover_days", default=None)):
        """
            Parameters
            ----------
            player_tag: player_tag to ban
            reason: reason for ban
        """
        await ctx.response.defer()
        banned_user: BannedResponse = await self.bot.ck_client.add_ban(server_id=ctx.guild_id, player_tag=player_tag, reason=reason, added_by=ctx.user.id, rollover_days=rollover_days)
