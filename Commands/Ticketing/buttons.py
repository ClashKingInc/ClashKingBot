    ...
    async def on_button_click(self, ctx: disnake.MessageInteraction):
        ...
        if ctx.data.custom_id == "approve_applicant":
            ...
            if not panel.approve_messages:
                return await ctx.send(content="No messages are set up, use `/ticket apply-messages`", ephemeral=True)
        ...
        if ctx.data.custom_id == "set_clan":
            ...
            dropdown = [disnake.ui.ActionRow(select)]
            msg = await ctx.followup.send(content="**Choose Clans To Send Player To:**", components=dropdown, ephemeral=True)
        ...
        if ctx.data.custom_id == "close_ticket":
            ...
            ticket = OpenTicket(bot=self.bot, open_ticket=result)
            if ctx.user.id == ticket.user:
                return await ctx.send(content="You don't have permissions to delete this ticket", ephemeral=True)

            await ticket.set_ticket_status(status="delete")
            await ctx.channel.delete()


        panel_settings = await self.bot.tickets.find_one({"$and": [{"server_id": ctx.guild.id}, {"name": ctx.data.custom_id.split("_")[0]}]})
        if panel_settings is not None:
            ...
            if button.account_apply:
                ...
                if not linked_accounts:
                    ...
                    for button in [
                            disnake.ui.Button(label="Link Account", emoji="🔗", style=disnake.ButtonStyle.green, custom_id="Start Link"),
                            disnake.ui.Button(label="Help", emoji="❓", style=disnake.ButtonStyle.grey, custom_id="Link Help"),
                        ]:
                ...
                for account in accounts:
                    ...
                    meet_hero_req = True
                    name_to_req = {"Barbarian King" : data_requirements.barbarian_king,
                ...
                message = await ctx.followup.send(content=f"Select Account to Apply With {text}", components=dropdown, ephemeral=True)

                res: disnake.MessageInteraction = await interaction_handler(bot=self.bot, ctx=ctx, msg=message, no_defer=(button.questions != []))
                ctx = res
            ...
            if len(players) >= 2:
                ...
                for account in players:
                    options.append(disnake.SelectOption(label=account.name, emoji=self.bot.fetch_emoji(name=account.town_hall).partial_emoji, value=f"ticketviewer_{account.tag}"))
                select = disnake.ui.Select(
                    options=options,
                ...
                dropdown = [disnake.ui.ActionRow(disnake.ui.Button(label="Applicant Account", emoji=self.bot.fetch_emoji(name=players[0].town_hall).partial_emoji,
                                                                   style=disnake.ButtonStyle.grey, custom_id=f"redditplayer_{players[0].tag}"))]

            next_action_row = disnake.ui.ActionRow()
            for b in [disnake.ui.Button(label="Delete Ticket", style=disnake.ButtonStyle.grey, custom_id=f"close_ticket"),
                      disnake.ui.Button(label="Set Clan",  style=disnake.ButtonStyle.grey, custom_id=f"set_clan"),
                      disnake.ui.Button(label="Messages", style=disnake.ButtonStyle.green, custom_id=f"approve_applicant"),
                      ]:
