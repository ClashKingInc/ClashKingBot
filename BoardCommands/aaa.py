async def create_donations(self, guild: disnake.Guild, type: str):
    ...
    if type == "donated":
        ...
        donated_text = [f"{self.bot.get_number_emoji(color='gold', number=count)} {line[0]}" for count, line in
                        enumerate(donated_text[:50], 1)]
        donated_text = "\n".join(donated_text)
        ...
        received_text = [f"{self.bot.get_number_emoji(color='gold', number=count)} {line[0]}" for count, line in
                         enumerate(received_text[:50], 1)]
        received_text = "\n".join(received_text)
        ...
        ratio_text = [f"{self.bot.get_number_emoji(color='gold', number=count)} {line[0]}" for count, line in
                      enumerate(ratio_text[:50], 1)]
        ratio_text = "\n".join(ratio_text)
...
async def create_last_online(self, guild: disnake.Guild):
    clan_tags = await self.bot.clan_db.distinct("tag", filter={"server": guild.id})
    member_tags = [r["tag"] for r in (
        await self.bot.player_stats.find({"clan_tag": {"$in": clan_tags}}).sort("last_online", -1).to_list(length=50))]
...
async def create_clan_games(self, guild: disnake.Guild):
    ...
    all_tags = [r["tag"] for r in (
        await self.bot.player_stats.find({"tag": {"$in": all_tags}}).sort(f"clan_games.{season_date}.clan", -1).to_list(
            length=50))]

    members = set(members)
    player_list = await self.bot.get_players(tags=list(set(all_tags)))

    member_stats = await self.bot.new_looper[f"{self.bot.gen_season_date()}-history"].find(
        {"tag": {"$in": all_tags}}).to_list(length=100)
    member_stat_dict = {}
    ...
    cg_point_embed = disnake.Embed(title=f"**{guild.name} Clan Game Totals**", description=point_text,
                                   color=disnake.Color.green())
...
async def create_trophies(self, guild: disnake.Guild, sort_type: TrophySort):
    ...
    if sort_type is TrophySort.home:
        ...
        clan_text = [f"{self.bot.emoji.trophy}`{clan.points:5} {clan.name}`{create_superscript(clan.member_count)}" for
                     clan in clans]
...
async def create_joinhistory(self, guild: disnake.Guild):
    ...
    for clan in clans:
        member_stats = await self.bot.new_looper[f"{self.bot.gen_season_date()}-history"].find(
            {"clan.value.tag": clan.tag}).to_list(length=100)
        all_items = []
