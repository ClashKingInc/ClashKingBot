    ...
    async def send_log(
        ...
        if log_type == LOG_TYPE.BUTTON_CLICK:
            embed = disnake.Embed(
                ...
                f' - Time: {self.bot.timestamp(unix_time=int(datetime.utcnow().timestamp())).cal_date}',
            ...
            embed = disnake.Embed(
                ...
                f' - By: {user.mention}\n'
                f' - Ticket: {ticket_channel.name}, {ticket_user.mention}({ticket_user})\n'
                f' - Time: {self.bot.timestamp(unix_time=int(datetime.now().timestamp())).cal_date}\n'
                f' - Ticket Creation: {self.bot.timestamp(unix_time=int(creation_time.timestamp())).cal_date}',
                color=disnake.Color(2829617),
            )
            embed.set_author(name=user.name, icon_url=user.display_avatar.url)
