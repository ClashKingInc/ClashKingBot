...
describe("first-slice command registry", () => {
  it("publishes only the implemented base and link command families", () => {
    expect(commandDefinitions.map((command) => command.name)).toEqual(["base", "link", "unlink", "roster"]);
  ...
  it("defers commands and completes the original interaction", async () => {
    const post = vi.fn(async () => ({ account: { is_verified: true, name: "Barbarian", tag: "#P0Y", townHallLevel: 1 }, message: "ok" }));
    ...
    interaction.type = 5;
    interaction.data = { custom_id: "ck:link:submit", components: [
      { type: 18, component: { type: 4, custom_id: "player", value: "P0Y" } },
      { type: 18, component: { type: 4, custom_id: "api_token", value: "valid-token" } },
    ] };
    const response = await dispatchInteraction(interaction, env, {
      api: { post, get: vi.fn(async () => ({ items: [] })) }, discordRest: { editOriginalInteractionResponse },
    } as unknown as CommandServices, (promise) => pending.push(promise));

    expect(response).toMatchObject({ type: 5, data: { flags: 64 } });
    await pending[0];
    expect(post).toHaveBeenCalledWith("/v2/links/100000000000000001", { api_token: "valid-token", player_tag: "#P0Y" });
